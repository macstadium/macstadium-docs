...
MacStadium recommends using a recipe when deploying a new VM, especially if you are new to this process. Recipes ensure that VMs are configured optimally for operation within Private Cloud, and are much faster to install and deploy when compared to manual VM creation.
...
MacStadium recommends using a preexisting recipe if a recipe exists for the OS required. Recipes always install the latest version of the selected OS in an optimal configuration with minimal post-installation configuration required. If there is not an OS recipe that you need, select New VM and use the template to create a new recipe for that OS.

4\. In the example, select the recipe for **Ubuntu Server 24.04** and click **Next.**
...
_VM Recipe Instance_
  ...
  7. In the Network box, select **Static** from the dropdown box. You must also select the correct **Network** from the dropdown list to ensure connectivity to existing MacStadium infrastructure. Otherwise, Private Cloud will provision a new isolated network for the VM. 
