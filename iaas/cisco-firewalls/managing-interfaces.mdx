...
To turn _logging_ on, you will first need to identity a repository for the log information. MacStadium recommends designating a separate repository for those log files, because it can grow quickly. This can be set in ASDM via **Monitoring > Logging**. Users can also submit a firewall change request via the MacStadium portal to choose a new repository.

Users running CI as a service, should port those logs to a separate server. Otherwise, speed, storage, and performance may become impacted. MacStadium also recommends leaving logging disabled, especially for internal-only customers.
