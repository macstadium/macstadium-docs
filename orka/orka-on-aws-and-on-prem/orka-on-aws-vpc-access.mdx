...
To SSH into a running VM, you need the VM's private IP address within the VPC. You can retrieve this from `orka3 vm list`.
...
aws ssm start-session \
  ...
  --parameters '{"host":["<VM_PRIVATE_IP>"],"portNumber":["8822"],"localPortNumber":["8822"]}'
```

Replace `<BASTION_INSTANCE_ID>` with your bastion instance ID and `<VM_PRIVATE_IP>` with the private IP of the Mac EC2 node running the VM (for example, `10.0.1.50`). The SSH port range for Orka VMs starts at `8822`.
...
The process for Screen Share and VNC access mirrors the SSH approach, using the appropriate port instead.
...
aws ssm start-session \
  --target <BASTION_INSTANCE_ID> \
  --document-name AWS-StartPortForwardingSessionToRemoteHost \
  --parameters '{"host":["<VM_PRIVATE_IP>"],"portNumber":["5900"],"localPortNumber":["5900"]}'
```
...
aws ssm start-session \
  --target <BASTION_INSTANCE_ID> \
  --document-name AWS-StartPortForwardingSessionToRemoteHost \
  --parameters '{"host":["<VM_PRIVATE_IP>"],"portNumber":["5999"],"localPortNumber":["5999"]}'
```

Once the tunnel is open, connect your Screen Share or VNC client to `localhost` on the forwarded port.

<Note>
  Screen Share and VNC must be enabled in macOS on the VM for these connections to work. If you haven't enabled them, you can do so via System Settings on the VM or through a startup script.
</Note>
