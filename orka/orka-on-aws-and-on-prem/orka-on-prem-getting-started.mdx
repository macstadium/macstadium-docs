...
<Note>
`orka3 vm push` requires an Orka API token. On-prem deployments often use certificate-based kubeconfig authentication, which is not sufficient for `vm push`. Before pushing VM images, authenticate with `orka3 login` or `orka3 user set-token`. An `Unauthorized` error from `vm push` indicates a missing Orka API token, not an OCI registry authentication issue.
</Note>
