...
So, instead of using a locally stored image, you can deploy directly from MacStadium's public OCI registry.
...
> This operation restarts the VM.
> 
> Run `orka3 vm save --help` for more information.
...
> 
> Run `orka3 vm push --help` for more information.
...
**Commit** (modifies the original base image):

```bash
orka3 vm commit <VM_NAME>
```

**Save** (creates a new base image):

```bash
orka3 vm save <VM_NAME> <NEW_IMAGE_NAME>
```

**Push** (sends the image to an OCI-compatible registry):
...
You can get the `<VM_NAME>` from the `orka3 vm list` output you ran earlier. Registry credentials are required to push an image — run `orka3 regcred --help` for more information.
