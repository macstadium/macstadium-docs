...
<Note>
**Before you begin:** From your [IP Plan](/macstadium/macstadium-overview/ip-plan), you need the `Outside` interface name, the `Private-1` IP address and subnet mask. From GCP, you need your local network's IPv4 address and subnet mask (converted from CIDR notation, e.g. `255.255.0.0` instead of `/16`).
</Note>
