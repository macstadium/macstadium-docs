...
Next, you need to upload the custom TLS certificate and private key to your cluster.
  ...
  2. Make sure you are authenticated with your cluster. Run `orka3 login` if you haven't already, or use a service account token.
...
```
curl -X POST '<ORKA_API_URL>/api/v1/upload/cert' \  
--header 'Content-Type: multipart/form-data' \  
--header 'Authorization: Bearer <TOKEN>' \  
--form 'certPath=@<full-path-to-the-certificate>' \  
--form 'keyPath=@<full-path-to-the-private-key>'
```
Replace `<ORKA_API_URL>` with your Orka API URL (for example, `http://10.221.188.20`).

Replace `<TOKEN>` with your Orka API token.
...
Finally, you need to target your external custom domain with your Orka tools. Note that you need to use https with your custom TLS domain.
  ...
  * For the Orka3 CLI, run `orka3 config set --api-url https://<custom-domain>`. Note that you can use only valid TLS certificates issued by a limited number of certificate authorities.
