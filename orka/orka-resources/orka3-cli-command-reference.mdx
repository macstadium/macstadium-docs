...
description: "Auto-generated Orka3 CLI command reference: all subcommands for VMs, images, nodes, namespaces, service accounts, RBAC, and OCI registry credentials."
...
# Create a new namespace named 'orka-cp' which allows running custom pods (previously sandboxing)  
...
Manage the service accounts, including any authentication tokens for them, for your CI/CD integrations. All operations in this group require administrative privileges.
...
# (Intel-only) Create a template for a VM with GPU passthrough (GPU passthrough must be enabled on the environment)  
...
* When both VM_NAME and '--generate-name' are provided, Orka generates a unique name based on the VM_NAME and a unique suffix.
...
# (Intel-only) Deploy a VM with GPU passthrough (GPU passthrough must be enabled on the environment)  
...
For Intel-based VMs ('amd64' architecture), you can provide SSH access credentials for Orka to complete the resize and repartition automatically. If you don't provide SSH access credentials, you need to complete the resize manually. See '/orka/orka3-cli-reference/vm-lifecycle-management'
