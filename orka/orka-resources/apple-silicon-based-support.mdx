---
title: "Apple silicon Mac servers in Orka"
description: "Run CI/CD on Apple silicon Mac servers with Orka. Create arm64 VMs, use image caching, and understand differences from Intel-based nodes."
...
Orka supports clusters of Apple silicon-based nodes (also: Apple ARM-based nodes) for running CI/CD workflows. Apple silicon-based nodes can be added to an existing cluster alongside Intel-based nodes, giving you a mixed Intel and Apple silicon cluster.

Supported VM operations on Apple silicon-based nodes include deploy, delete, and list, as well as connecting to a VM via SSH or Screenshare.
...
Orka 3.3:
  ...
  * Hardware UUID generation control (3.3.2): consistent or dynamic Hardware UUIDs configurable per cluster
...
<Warning>
You can deploy up to 2 VMs per Apple silicon-based node. This is an Apple EULA requirement and applies to all Apple silicon hardware generations.
</Warning>

  1. The following operations are not supported on Apple silicon-based nodes:

  * VMs: stop, start, suspend, resume (these are Intel-only operations due to Apple Virtualization Framework limitations)
...
Below is a list of the ones that are not applicable to Apple silicon-based VMs:
  ...
  * Bring your own macOS serial number feature
  ...
  5. Shared VM storage on Apple silicon-based nodes requires macOS Sequoia or later.
