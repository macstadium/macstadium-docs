---
title: "Shared VM storage for Orka CI artifacts"
description: "Mount a shared storage volume across Orka VMs for CI artifact caching. Deprecated on Apple silicon Monterey; available for Intel and Apple silicon Ventura+."
...
<Note>
To use shared VM storage with VMs deployed on ARM nodes, your image must include Orka VM Tools. Orka VM Tools is required for shared storage to automount in the VM.
...
To fix this issue, connect to the VM via VNC and navigate to **Full Disk Access** (macOS Ventura and later: System Settings → Privacy & Security → Full Disk Access; macOS Monterey: System Preferences → Security & Privacy → Privacy → Full Disk Access). Click the padlock to make changes:
