...
General information and guidance on Scheduled Caching
  ...
  * **Can I delete images from a cluster node cache?**
    * Use `orka3 imagecache remove` to remove a cached image from one or more nodes. This operation requires admin privileges. To update an image without removing it first, cache a new copy under the same name or with an updated OCI tag.
