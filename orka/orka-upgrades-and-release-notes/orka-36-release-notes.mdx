...
Orka 3.6.3 introduces programmatic image cache removal via CLI and API, completing the `imagecache` command surface. This release also patches 15 CVEs in Go networking libraries, improves ARM node tooling upgrades for AWS deployments, and includes several Upgrade Service reliability fixes.
...
* Removal is blocked if a VM deployment is actively in-flight on the target node
...
Removal is also blocked if a VM deployment is actively in-flight on the target node.
...
* Fixed: Release workflow no longer appends a version entry if it already exists in the bundle
* Fixed: Agent and operator image versions in the generated bundle were incorrect
* Fixed: Master and release branch bundles were overwriting each other on the dev S3 path
