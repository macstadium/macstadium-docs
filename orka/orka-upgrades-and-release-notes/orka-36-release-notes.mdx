...
Orka 3.6.3 introduces programmatic image cache removal via CLI and API, completing the `imagecache` command surface. This release also patches 15 HIGH-severity CVEs in Go networking libraries, improves ARM node tooling upgrades for AWS deployments, and includes several Upgrade Service reliability fixes.
...
For usage and examples, see [Image caching](/orka/orka-resources/image-caching) and the [orka3 CLI reference](/orka/orka3-cli-reference/image-management#orka3-imagecache-remove).
