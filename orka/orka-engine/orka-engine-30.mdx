...
```
  * Delete VMs from local file system. Create and deploy Orka OCI compliant Virtual Machine images.
...
In previous examples, `orka-engine vm run` was used to both _pull_ (or download) a remote image to the local device and to _run_ the VM. There are cases where you may just want to download an image, and `orka-engine image pull` supports this. 
