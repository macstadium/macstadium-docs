...
keywords: ["Ansible commands", "playbook quick reference", "Orka CLI", "VDI command reference"]
---
...
```
ansible-playbook -i dev/inventory install_engine.yml \
...
```
ansible-playbook -i dev/inventory deploy_avd.yml \
  -e "vm_name=my-vm" \
  --tags plan
```
### Create an AVD
...
```
# Start AVD
ansible-playbook -i dev/inventory avd.yml \
  -e "vm_name=my-vm" \
  -e "desired_state=running"
...
```
ansible-playbook -i dev/inventory delete_avd.yml \
  -e "vm_name=my-vm" \
  -e "avd_index=0"
```
