...
<Note>
  Verify the log path on a live host before relying on this. The path below reflects current understanding but has not been confirmed against a running installation.
</Note>
...
<Note>
  The log path and filenames below are based on Citrix documentation but should be verified on a VM with VDA installed before relying on them.
</Note>
