...
description: "Step-by-step guide to deploying MacStadium VDI on your own infrastructure, on-premises or on AWS. Covers hardware preparation, networking, Orka Engine installation, and first VM deployment."
keywords: ["self-hosted macOS VDI", "on-premises VDI setup", "AWS Mac VDI deployment", "Ansible orchestration", "Orka Engine self-hosted"]
---
...
<Tabs>
  ...
  <Tab title="AWS">
    ...
    - Security group rules permitting the same Citrix Cloud outbound traffic listed above
...
```bash
brew install pipx
pipx install ansible-core==2.20.3
...
ansible-playbook install_engine.yml \
  -i inventory \
  -e "orka_license_key=YOUR-LICENSE-KEY" \
  -e "engine_url=YOUR-INSTALLER-URL"
```
...
The management UI (built on [Semaphore](https://www.semui.co)) is the primary interface for IT administrators. It provides a browser-based dashboard for running orchestration playbooks without touching the CLI.
...
```bash
SEMAPHORE_ADMIN=$YOUR_ADMIN SEMAPHORE_ADMIN_PASSWORD=$YOUR_ADMIN_PASSWORD \
  uv run ./semaphore/configure_semaphore.py --ssh-key-file $YOUR_KEY
```

Run `uv run ./semaphore/configure_semaphore.py --help` to see all options, including VM credentials and <Tooltip tip="Open Container Initiative — an industry standard for container image formats. Orka stores VM images in OCI-compatible registries like GHCR or Harbor.">OCI</Tooltip> registry settings.
...
ansible-playbook deploy.yml \
  -i inventory \
  -e "vm_name=vdi-test-01" \
  -e "vm_image=ghcr.io/macstadium/orka-images/sequoia:latest" \
...
<CardGroup cols={2}>
  <Card title="Citrix DaaS Configuration" href="/remote-desktop-vdi/configuration/citrix-daas-configuration">
    Register your VMs with Citrix Cloud and configure delivery groups.
  </Card>
  <Card title="Image Management" href="/remote-desktop-vdi/configuration/image-management">
