...
description: "Step-by-step guide to deploying MacStadium VDI on MacStadium-managed infrastructure. Your Mac hosts are ready: this guide covers connecting, configuring, and deploying your first VMs."
keywords: ["MacStadium hosted setup", "MSDC VDI deployment", "Ansible controller setup", "Orka Engine installation", "VPN setup macOS VDI"]
---
...
Run `uv run ./semaphore/configure_semaphore.py --help` to see all available options, including VM credentials and <Tooltip tip="Open Container Initiative — an industry standard for container image formats. Orka stores VM images in OCI-compatible registries like GHCR or Harbor.">OCI</Tooltip> registry settings.
