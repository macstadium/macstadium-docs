...
This workflow depends on Automated Device Enrollment (ADE) via [Apple Business Manager](https://support.apple.com/guide/apple-business-manager) (ABM), which requires physical hardware registered to your ABM account.
...
The machine reboots, re-enrolls, and the policy runs automatically. The temporary user account is created, SSH is enabled, and Screen Sharing is enabled. Hand the temporary credentials to the user. They can connect via VNC immediately and will be prompted to set a new password on first login.
