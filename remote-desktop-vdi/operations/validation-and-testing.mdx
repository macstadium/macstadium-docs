...
description: "Validate your Citrix + Orka VDI setup before production: end-to-end connectivity, VM lifecycle, performance baseline, and smoke tests via Citrix Workspace."
zendesk_id: 44332879842587
keywords: ["VDI validation", "deployment testing", "Citrix VDA verification", "performance baseline", "VDI acceptance testing"]
---
...
##### Disconnect and reconnect
  ...
  3. Verify work is preserved (previously opened documents are still open)
...
These operations form the foundation of desktop pool management with MacStadium VDI. When testing, the following lifecycle phases should be validated:
...
```
You will want to confirm that the VM stops gracefully (clean shutdown), that the VM state changes to “Stopped” in MacStadium VDI, the desktop becomes unavailable in Citrix Workspace, and that no active sessions remain.  
...
```
You will want to confirm the VM has been deleted from MacStadium VDI, the VM has been unregistered from Citrix Cloud, the desktop is removed from Citrix Workspace, resources have been freed on the Orka host.
...
Some metrics to consider measuring are:
  ...
  1. VM deployment time
     ...
     - To establish a baseline, record the average, minimum, and maximum time across 10+ deployments and set an acceptable target (for example, &lt;5 minutes for cached images).
...
**Expected result:** The host-side emulator appears as a connected device (for example, `emulator-5554 device`).
