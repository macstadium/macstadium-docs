...
keywords: ["port requirements", "firewall rules VDI", "HDX ports", "Citrix endpoints", "VPN macOS VDI", "network topology"]
---
...
<Note>
  The port and firewall requirements below cover Citrix DaaS, which is MacStadium's primary supported delivery platform. If you're using a different VDI broker, consult that platform's networking documentation for the equivalent port and endpoint requirements.
</Note>
...
### Citrix <Tooltip tip="Virtual Delivery Agent — software installed on each macOS VM that registers with the Delivery Controller and handles the remoting protocol session.">VDA</Tooltip> and <Tooltip tip="High Definition Experience — Citrix's remoting protocol for carrying screen, keyboard, mouse, and audio data between the VDA and the user's client.">HDX</Tooltip> traffic

These rules apply to all deployment models using Citrix DaaS.

| Traffic | Source | Destination | Protocol / Port | Purpose |
| --- | --- | --- | --- | --- |
| HDX sessions | End-user clients | macOS VMs | TCP/UDP 1494 | ICA/HDX protocol |
| HDX sessions (<Tooltip tip="Citrix Gateway Protocol — used for Session Reliability, which lets sessions survive brief network interruptions without dropping the connection.">CGP</Tooltip>) | End-user clients | macOS VMs | TCP/UDP 2598 | Session Reliability |
...
| Traffic | Source | Destination | Protocol / Port | Purpose |
| --- | --- | --- | --- | --- |
| Ansible → Orka hosts | Ansible controller | Mac hosts | TCP 22 | SSH (playbook execution and CLI) |
...
### <Tooltip tip="Open Container Initiative — an industry standard for container image formats and distribution. Orka stores VM images in OCI-compatible registries like GHCR or Harbor.">OCI</Tooltip> registry
