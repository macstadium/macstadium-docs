...
description: "Troubleshoot MacStadium Cloud Access connection issues: HP Anyware PCoIP configuration problems, a known bandwidth bug, and upgrade steps to resolve it."
...
Versions 22.07 - 23.12.0 contain a bug that significantly reduced the amount of bandwidth the PCoIP Graphics Agent utilizes. To upgrade to the latest PCoIP Graphics Agent and Software Clients, please visit HP Anyware.
...
If the above 6405 error or the network connection lost error persists through reboots and the proper ports above are opened up, or if the client has graphical issues such as pixellation or low quality, there may be connectivity issues between the client and the agent. To check for connectivity issues you should run a trace route, a speed test, and a ping test.
...
Internet speed tests are invaluable tools that provide an accurate measurement of your current download, upload, and latency speeds. This information helps you assess whether your internet service provider (ISP) delivers the advertised speeds and identify potential performance issues. With this data, you can make informed decisions about your internet plan or troubleshoot connectivity problems.
