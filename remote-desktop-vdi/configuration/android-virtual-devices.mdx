...
keywords: ["Android Virtual Device", "AVD macOS", "macOS Android emulator", "Orka AVD", "mobile testing VDI"]
---
...
- MacStadium VDI is deployed and operational (see the [Deployment Guide](/remote-desktop-vdi/getting-started/self-hosted))
...
AVDs are tied to a specific VM. The AVD name is derived automatically from the VM name using the pattern `{vm_name}-avd-{index}`, where the index increments for each additional AVD on that VM (for example, `my-vm-avd-0`, `my-vm-avd-1`).
...
- `avd_index`: index of the AVD to delete (for example, `0` for `my-vm-avd-0`)
